...
//serach in array
  ...
  bl BINARY_SEARCH_ARRAY   //return output in r0
  ldr r1, =OUTPUT
  str r0, [r1]      //store result in OUTPUT
exit:
  SWI SWI_Exit

// In worst case binary search take log N iteration to search a element in sorted array
.text
BINARY_SEARCH_ARRAY:
  STMFD SP!,{LR}
  mov r7, #0               //start
  mov r8, r1               //end
  mov r9, r1, lsr #1       //mid = N/2

loop:
  ldr r4, [r0, r9, lsl #2]
  cmp r4, r2
  beq found
  bgt left                 //if arr[mid] > search element then search in left

right:
  mov r7, r9               // start = mid

mid:
  sub r9, r8, r7           // mid = (end - start)/2
  mov r9, r9, lsr #1
  cmp r7, r8               // if start <= end continue serach
  ble loop
  b not_found
...
not_found:                 //if element not found then return -1
  mvn r9, #0
